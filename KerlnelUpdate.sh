...
read -p " Ingrese nombre completo de la instancia   " NAME_INSTANCE
read -p " Ingrese ruta completa de donde estan alojados los nuevos medios   " RUTA_FILES_NEW
...
EOF
                                                        ...
                                                        if [ "$type_system" == "FAIL" ];then
                                                                SSHPASS=$PASS sshpass -e $pass ssh -o "StrictHostKeyChecking no" $USER@$IP -p $port_con_ic "cd $rute && startsap R3" 2>&1 >/dev/null
                                                        else
                                                               SSHPASS=$PASS sshpass -e $pass ssh -o "StrictHostKeyChecking no" $USER@$IP -p $port_con_ic "cd $rute && startsap" 2>&1 >/dev/null
                                                        fi
